  ...
  useEffect(() => {
    const fetchCampuses = async () => {
      try {
        ...
        setCampuses([]);
        setDropdownError(error?.message || 'Unable to load campus dropdown data.');
      } finally {
        setLoading(false);
      }
    };
        ...
        <div style={S.authFormPanel}>
          <div style={{ ...S.authFormInner, maxWidth: '460px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              {/* Campus → College → Department cascading dropdowns */}
              <select 
                style={{ ...S.inp, ...S.sel }} 
                value={campus} 
              ...
              >
                <option value="">
                  {!campus ? 'First select campus' : 
                   filteredColleges.length === 0 ? 'No colleges available' : 
                   'Select College *'}
                </option>
              ...
              <button 
                style={{ 
                  ...S.submitBtn, 
                  opacity: (loading || campuses.length === 0 || filteredColleges.length === 0 || !campus || !college || !dept) ? 0.6 : 1,
                  cursor: (loading || campuses.length === 0 || filteredColleges.length === 0 || !campus || !college || !dept) ? 'not-allowed' : 'pointer'
                }} 
                onClick={handleSignup}
                disabled={loading || campuses.length === 0 || !campus || (campus && filteredColleges.length === 0) || !college || !dept}
              >
                {loading ? 'Loading...' : 
                 (campuses.length === 0) ? 'Backend Required' : 
                 'Sign Up'}
              </button>
